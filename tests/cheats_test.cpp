...
int main() {
    ...
    down(menu); right(menu); // Lighting: on by default, so this turns it off.
    assert(!menu.settings().dynamicLighting);
    down(menu); right(menu); // Weather.
    assert(menu.settings().weather);
    down(menu); tap(menu, &PlayerControlsState::c); // Restore defaults.
    assert(!menu.settings().enhancedGraphics && !menu.settings().smoothAnimation && menu.settings().dynamicLighting && !menu.settings().weather);
